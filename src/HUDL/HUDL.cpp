 ...
 */
    void HUDL::drive_pixel(unsigned char page, unsigned char col_up,
        ...
        HUDL::comm_write(0x40); // line to start writing on (0 -> 64) moves set bits with it
        // DO NOT CHANGE
        comm_write(0xB0 + page); // writes the page address (4 bits, 8 rows selected by values 0-7 )
 ...
 */
    void HUDL::init_LCD() {
        ...
        HUDL::comm_write(0x81); // Electronic Volume Command (set contrast) Double Byte: 1 of 2
        HUDL::comm_write(0x11); // Electronic Volume value (contrast value) Double Byte: 2 of 2
